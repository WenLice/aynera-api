...
- Interactive (Development only): `/swagger` — three documents, selectable from the top-right dropdown. **Aynera API (all endpoints)** (`/swagger/all/swagger.json`) opens by default and lists every endpoint, member and staff, so nothing is hidden behind a dropdown; **Aynera Member API** (`/swagger/member/swagger.json`: public and member endpoints) and **Aynera Admin API** (`/swagger/admin/swagger.json`: staff endpoints plus admin login) are the per-audience views each front end codes against. An endpoint's audience is derived from its authorization policy (`Admin`/`SuperAdmin` → Admin doc), so the split cannot drift from what the backend enforces; the combined document is the union of the two and changes nothing about authorization — staff endpoints still require an admin token. `ControllerRoutingTests` asserts the combined document holds every endpoint and that exactly one audience document claims each.
...
| `gender` | `string` | Yes | `Male` \| `Female` \| `ThirdGender` (shown as "Third Gender / Transgender") — the only three values |
| `genderIsPublic` | `boolean` | No | Defaults `true`. `false` hides the gender on the profile; it is still used for reciprocal matching. Stored as `visibility.gender` in member settings |
...
| `gender` | `string` | |
| `genderIsPublic` | `boolean` | False when the member chose not to show it. Still used for reciprocal matching |
| `dateOfBirth` | `date` | |
...
| GetMySettings | `GET` | `/members/me/settings` | `Member` | — | `MemberSettingsDto` |
| UpdateMySettings | `PATCH` | `/members/me/settings` | `Member` | `UpdateMemberSettingsRequest` | `MemberSettingsDto` |
| UploadVoiceAnswer | `POST` | `/voice-answers/Upload` | `Member` | multipart `audio` + `promptId` | `VoiceAnswerDto` |
| ListVoiceAnswers | `GET` | `/voice-answers/GetAll` | `Member` | — | `VoiceAnswerDto[]` |
| DeleteVoiceAnswer | `DELETE` | `/voice-answers/{promptId}` | `Member` | — | `null` |
| GetMemberVoiceAnswer (staff) | `GET` | `/voice-answers/{id}/{promptId}/content` | `Admin` | — | audio bytes |
...
**Conversation prompts (the `voice` step).** The member chooses 2 prompts (a 3rd from the profile editor) and answers each by typing, recording, or both. The list is saved with `PATCH members/me/registration` as `prompts: [{ promptId, text? }]` — the full list in order, replacing what is stored; `promptId` is the app's key (`^[a-z0-9][a-z0-9_-]{0,31}$`), `text` ≤ 300 chars. A spoken answer is uploaded with `POST voice-answers/Upload` (form `promptId` + file `audio`; M4A/AAC, MP3, WebM or Ogg, ≤ 5 MB — the app caps recordings at 60 s) and stored at `{userId}/voice_{promptId}.{ext}` as a `MemberMedia` row of kind `VoiceAnswer`. Errors: `voice_prompt_not_chosen` (upload for a prompt not in the list), `voice_prompt_invalid`, `voice_unsupported_type`, `voice_too_large`, `voice_empty`, `voice_guideline_failed`, 404 `voice_not_found`. Removing a prompt from the list removes its recording. The step is done once 2 prompts are answered; `profileAnswers.prompts[].hasAudio` says which have a recording. `GET voice-answers/GetAll` returns `{ promptId, contentType, byteSize, url, createdAtUtc, updatedAtUtc }` with a signed link valid one hour.

**Member settings (`MemberSettings`, one row per member).** `GET members/me/settings` and `PATCH members/me/settings` (Member) hold everything about how Aynera treats the member: `notifyIntroductions`, `notifyReplies`, `notifyWeekendSurprise` (null until answered), `introductionsPaused` + `pausedAtUtc` (set when the pause starts, kept on repeat pauses, cleared on resume), and `visibility` — which profile fields show, e.g. `{ "gender": false, "lifestyle.drink": true, "beliefs.faith": false }`; a field with no entry is shown. PATCH is partial and `visibility` merges per field; an empty body or a malformed field name is 400 `validation_failed`. The gender's visibility is the same stored choice as `genderIsPublic` on the profile, and `lifestyle.*` / `beliefs.*` are the same as each answer's `public` flag — the profile and answer endpoints still accept and return those, reading and writing the settings row. On the registration PATCH, `notificationsOn: true|false` sets all three notify switches and completes the `notifications` step.

**Profile extras.** `dealbreaker` (≤ 500 chars, empty clears) and `rhythm` (`{ "socialEnergy": "…", "weekends": "…", "family": "…" }`, replaces) come from the profile editor and are returned in `profileAnswers`.
