...
public sealed class UserRepository : IUserRepository
{
    ...
    public async Task<UserRecord?> FindByPhoneAsync(string phoneE164, CancellationToken cancellationToken)
    {
        ...
        var user = await _userManager.Users.AsNoTracking()
            .FirstOrDefaultAsync(u => u.UserName == phoneE164, cancellationToken);
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        // Authorization may have read this account before a workflow waited for its lock.
        // A projection must reflect committed state, not an earlier tracked Identity entity.
        var user = await _userManager.Users.AsNoTracking()
    ...
    public async Task SetConfirmedEmailAsync(Guid userId, string email, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        _logger.LogDebug("SetConfirmedEmailAsync {UserId}", userId);
        var user = await RequireActiveLifecycleUserAsync(userId);
        var trimmed = email.Trim();

        var owner = await _userManager.FindByEmailAsync(trimmed);
        if (owner is not null && owner.Id != user.Id && !owner.IsDeleted)
        {
            throw new AuthException(
                "email_already_exists",
                "A member account with this email already exists.",
        ...
        if (!result.Succeeded)
        {
            if (result.Errors.Any(e => e.Code.Contains("Email", StringComparison.Ordinal)))
            {
                throw new AuthException(
                    "email_already_exists",
                    "A member account with this email already exists.",
