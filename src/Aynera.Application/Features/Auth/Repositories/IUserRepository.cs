...
public interface IUserRepository
{
    ...
    /// <summary>
    /// Sets (or replaces) the account email and marks it confirmed in one write — used once the
    /// member has proven the address with a code. Throws <c>email_already_exists</c> (409) when
    /// another live account owns it.
    /// </summary>
    Task SetConfirmedEmailAsync(Guid userId, string email, CancellationToken cancellationToken);
